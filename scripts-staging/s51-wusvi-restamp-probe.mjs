#!/usr/bin/env node
// s51-wusvi-restamp — PROBE stage. Copy of scripts-staging/s50-wusvi-refresh-probe.mjs (vendored from wanderamsterdam scripts/s49-wams-refresh-probe.mjs
// (itself wanderengland s48-weng-refresh-b.mjs probe()), byte-for-byte except: population source, EV path, DATES.
//   Population: the 282 pks frozen in scripts-staging/evidence/s51-wusvi-restamp/population.json
//   (live rows with no s50 dated stamp: the #135-#145 ruled rows + the s40 HOLD rows). Asserted against the data file at run time.
...
//   APPLY stage: scripts-staging/s51-wusvi-restamp-apply.py (Python — the file carries float spellings).
//   usage: node scripts-staging/s51-wusvi-restamp-probe.mjs
import fs from 'node:fs';
const FILE = 'tours-data.json';
const EV = 'scripts-staging/evidence/s51-wusvi-restamp';
