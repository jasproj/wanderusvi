#!/usr/bin/env python3
"""S44 - apply the adjudicated max-tier charter-sweep prices to tours-data.json.

Deterministic (D-581, D-599 byte round-trip). The only input is the frozen ledger
scripts-staging/s44-charter-max-tier-adjudicated.json. Re-running on an
...
Usage: python3 scripts-staging/apply-s44-charter-max-tier.py [--dry-run]
"""
import json
import os
import sys
...
LEDGER = os.path.join(ROOT, "scripts-staging", "s44-charter-max-tier-adjudicated.json")
