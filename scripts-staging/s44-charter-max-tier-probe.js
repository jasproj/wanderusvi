/**
 * S44 recon probe — live FareHarbor evidence for the max-tier charter sweep (same instrument as S43).
 *
 * Recon only: writes nothing into the repo. Reproduces the evidence behind
 * scripts-staging/s44-charter-max-tier-adjudicated.json.
 *
 * Population : priceLabel=="charter" rows (extract-price-v5.2.js Math.max branch, D-600).
 ...
 *
 * Usage: node scripts-staging/s44-charter-max-tier-probe.js <outputDir> [BASE-DATE]
 */
const fs=require("fs");
const SP=process.argv[2];
if(!SP){ console.error("usage: node s44-charter-max-tier-probe.js <outputDir> [BASE-DATE]"); process.exit(1); }
